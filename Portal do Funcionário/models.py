...
def calcular_horas_ponto(ponto, carga_diaria=0, limite_extras=2):
    marcacoes = sorted(
        ponto.marcacoes,
        key=lambda m: m.hora)
    ...
    carga = timedelta(hours=limite_extras)
...
class Ponto(db.Model):
    ...
    marcacoes = db.relationship("Marcacao", backref="ponto", lazy=True, cascade="all, delete-orphan")
